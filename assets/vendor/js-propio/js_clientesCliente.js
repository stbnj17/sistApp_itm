$(document).ready(function() {
	...
	$('#datatableEmpresas').DataTable().columns([0]).visible(false);
	...
	$('#datatablePersonas').DataTable().columns([0]).visible(false);
...
function agregarEmpresa(dataForm) {
	$.ajax({
		url: 'admClientesCliente/nuevaEmpresa',
		type: 'POST',
		data: dataForm,
	})
	.success(function(info) {
		...
		switch(json) {
			case 'ok':
				window.location.href = 'admClientesCliente';
				break;
			default:
...
function verEmpresa(id) {
	$.ajax({
		url: 'admClientesCliente/verEmpresa',
		type: 'POST',
		data: {id: id},
	})
	.success(function(info) {
...
function editarEmpresa(dataForm) {
	$.ajax({
		url: 'admClientesCliente/modificarEmpresa',
		type: 'POST',
		data: dataForm,
	})
	.success(function(info) {
...
function eliminarEmpresa(id) {
	$.ajax({
		url: 'admClientesCliente/eliminarEmpresa',
		type: 'POST',
		data: {id: id},
	})
	.success(function() {
...
function agregarPerNatural(dataForm) {
	$.ajax({
		url: 'admClientesCliente/nuevaPerNatural',
		type: 'POST',
		data: dataForm,
	})
	.success(function(info) {
...
function verPerNatural(id) {
	$.ajax({
		url: 'admClientesCliente/verPerNatural',
		type: 'POST',
		data: {id: id},
	})
	.success(function(info) {
		...
		for (var i = 0; i < ePnCustomer.length; i++) {
      var opcion = ePnCustomer.options[i].value;
      if (opcion == json[0].codCu) {
        ePnCustomer.options[i].selected=true;
      };
    };
...
function editarPerNatural(dataForm) {
	$.ajax({
		url: 'admClientesCliente/modificarPerNatural',
		type: 'POST',
		data: dataForm,
	})
	.success(function(info) {
...
function eliminarPerNatural(id) {
	$.ajax({
		url: 'admClientesCliente/eliminarPerNatural',
		type: 'POST',
		data: {id: id},
	})
	.success(function() {
